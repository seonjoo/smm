...
function(data, index, thresh = 0.0001,
  ...
  for(k in 1:nlam){
    ...
    junk <- .C("linNestPen", X = as.double(as.vector(X)), y = as.double(y),
               ...
               lambda1 = as.double(lambdas[k]*penalty.factor*alpha),
               lambda2 = as.double(lambdas[k]*(1-alpha)* group.penalty.factor),
