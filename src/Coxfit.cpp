...
// modified code
void linSolverPen(double *X, double *y, int* index, int *nrow, int *ncol, int *numGroup, double *beta, int *rangeGroupInd, int *groupLen, double *lambda1, double *lambda2, int *innerIter, double *thresh, double *ldot, double *nullBeta, double *gamma, double *eta, int* betaIsZero, int& groupChange, int* isActive, int* useGroup, double *step, int *reset)
{
  double *theta = new double[ncol[0]];
  int startInd = 0;
  double zeroCheck = 0;
  ...
  for(int i = 0; i < numGroup[0]; i++)
  {
    if(useGroup[i] == 1)
    {
      ...
      for(int j = 0; j < groupLen[i]; j++)
      {
        ...
        if(grad[j] < lambda1[j] && grad[j] > -lambda1[j])
        {
          grad[j] = 0;
        }
        if(grad[j] > lambda1[j])
        {
          grad[j] = grad[j] - lambda1[j];
        }
        if(grad[j] < -lambda1[j])
        {
          grad[j] = grad[j] + lambda1[j];
        }
        if(pow(grad[j],2) == pow(lambda1[j],2))
        {
          grad[j] = 0;
        }
      ...
      if(zeroCheck <= pow(lambda2[i],2)*groupLen[i])  //Or not?
      {
        if(betaIsZero[i] == 0)
        {
          for(int k = 0; k < nrow[0]; k++)
      ...
      else
      {
        ...
        while(count <= innerIter[0] && check > thresh[0])
        {
          ...
          while(diff < 0)
          {
            ...
            {
              z[j] = beta[j + rangeGroupInd[i]] - t * grad[j];
              if(z[j] < lambda1[j] * t && z[j] > -lambda1[j] * t)
              {
                z[j] = 0;
              }
              if(z[j] > lambda1[j] * t)
            ...
            if(norm != 0){
              uOp = (1 - lambda2[i]*sqrt(double(groupLen[i]))*t/norm);   //Or not?
            }
            else{uOp = 0;}
...
int linNestPen(double *X, double* y, int* index, int *nrow, int *ncol, int *numGroup, int *rangeGroupInd, int *groupLen, double *lambda1, double *lambda2, double *beta, int *innerIter, int *outerIter, double *thresh, double *outerThresh, double *eta, double *gamma, int *betaIsZero, double *step, int *reset)
{
  double* prob = NULL;
  prob = new double[nrow[0]];
  double* nullBeta = NULL;
  ...
  while(groupChange == 1)
  {
    ...
    linSolverPen(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd, groupLen, lambda1, lambda2, innerIter, thresh, ldot, nullBeta, gamma, eta, betaIsZero, groupChange, isActive, useGroup, step, reset);
    ...
    while(outermostCounter < outerIter[0] && outermostCheck > outerThresh[0])
    {
      ...
      linSolverPen(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd, groupLen, lambda1, lambda2, innerIter, thresh, ldot, nullBeta, gamma, eta, betaIsZero, groupChange, isActive, tempIsActive, step, reset);
